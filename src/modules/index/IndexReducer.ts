...
import { makeAction, isAction, DEFAULT_ACTION } from '../../redux/guards';
...
export const setTitle = makeAction(SET_TITLE)((title: string) => ({
  type: SET_TITLE,
  payload: title,
}));
export const saveTodo = makeAction(SAVE_TODO)(() => ({ type: SAVE_TODO }));
export const saveTodoSuccess = makeAction(SAVE_TODO_SUCCESS)(() => ({
  type: SAVE_TODO_SUCCESS,
}));
export const setDone = makeAction(SET_DONE)((i: number) => ({
  type: SET_DONE,
  payload: i,
}));
export const setDoneSuccess = makeAction(SET_DONE_SUCCESS)((i: number) => ({
...
const IndexReducer = (
  state: IndexState = new IndexState(),
  action: Action,
): IndexState => {
